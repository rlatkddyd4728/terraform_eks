resource "aws_iam_policy" "karpenter" {
  ...
  name   = format("%s_%s_%s_karpenter_controller_policy",var.prefix,var.env,var.role)
  policy = <<JSON
{
    "Statement": [
        {
...
resource "aws_iam_policy" "karpenter_node" {
  count  = local.enable.karpenter ? 1 : 0
  name   = format("%s_%s_%s_karpenter_node_policy",var.prefix,var.env,var.role)
  policy = <<JSON
{
    "Version": "2012-10-17",
    "Statement": [
...
resource "aws_iam_role" "karpenter_node" {
  count                 = local.enable.karpenter ? 1 : 0
  name                  = format("%s_%s_%s_karpenter_node",var.prefix,var.env,var.role)
...
resource "aws_iam_instance_profile" "karpenter_node" {
  count         = local.enable.karpenter ? 1 : 0
  name          = format("%s_%s_%s_karpenter_node_profile",var.prefix,var.env,var.role)
  role          = aws_iam_role.karpenter_node[0].name
