resource "aws_iam_role" "eks_cluster" {
  name                  = format("%s_%s_%s_eks_cluster",var.prefix,var.env,var.role)
...
resource "aws_iam_policy" "eks_node" {
  name   = format("%s_%s_%s_eks_node_policy",var.prefix,var.env,var.role)
  policy = <<JSON
{
    "Version": "2012-10-17",
    "Statement": [
...
resource "aws_iam_role" "eks_node" {
  name                  = format("%s_%s_%s_eks_node",var.prefix,var.env,var.role)
  assume_role_policy    = <<JSON
{
    "Version": "2012-10-17",
    "Statement": [
